...
df <- read_csv("../networks/union_nodes.csv")
...
# Rimuovere proteins without GO annotations (if any)
df_selected <- df_selected %>% filter(lengths(GO_list) > 0)

# Init objects for similarity calculations:
#   - Biological Process
#   - Molecular Function
#   - Cellular Component
...
# Process similarity between proteins using annoted GO terms with Lin similarity metric.
# Then aggregate with Best Matching Average
...
# Only needed half of the comparisons (matrix's upper triangle)
index_pairs <- expand.grid(1:n, 1:n)
index_pairs <- index_pairs[index_pairs$Var1 <= index_pairs$Var2, ]

print("Started...")
start_time <- Sys.time()
total_comparisons <- nrow(index_pairs)
...
progress_log <- function(counter, total) {
  ...
  log_message <- (sprintf("Progress: %d/%d (%.2f%%) | Time ellapsed: %.2f min | Remaning Estimated Time: %.2f min",
...
results <- bplapply(1:nrow(index_pairs), function(k) {
  if (k %% 100 == 0) progress_log(k, total_comparisons)
